    ...
    <main className="bg-white text-gray-900 dark:bg-slate-900 dark:text-gray-100">
      ...
      <section id="categories" className="section">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { title: "Sedans", img: "/sedan.jpg", desc: "Comfortable, fuel-efficient cars for city and highway driving.", href: "/sedans" },
            { title: "SUVs", img: "/suv.jpg", desc: "Spacious and versatile, perfect for families or road trips.", href: "/suvs" },
            { title: "3-Row SUVs", img: "/3row-suv.jpg", desc: "Extra seating and storage for larger groups and longer journeys.", href: "/3row-suvs" },
          ].map((cat) => (
            <div key={cat.title} className="group rounded-xl overflow-hidden border bg-white dark:bg-slate-800 hover:shadow-lg transition flex flex-col">
              <div className="relative h-40">
                <Image src={cat.img} alt={cat.title} fill className="object-cover" />
              </div>
              <div className="p-4 flex flex-col flex-1 text-center">
                <h3 className="text-lg font-medium">{cat.title}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-300 flex-1">{cat.desc}</p>
                <Link
                  href={cat.href}
      ...
      <section className="section bg-gray-50 dark:bg-slate-800 text-center">
        ...
        <div className="grid md:grid-cols-4 gap-6 max-w-5xl mx-auto">
          {[
            { icon: "📅", title: "1. Pick Your Dates & Car", desc: "Select your rental period and vehicle that best fits your trip." },
            { icon: "💰", title: "2. Get Instant Quote", desc: "Our system gives you a transparent price right away." },
      ...
      <section className="section">
        <div className="max-w-3xl mx-auto bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-600 shadow-md rounded-lg p-6 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-6 text-left">
            {[
              { review: "Ashmaig Auto Rentals made everything simple. The SUV was spotless and pickup was fast!", author: "— Sarah K." },
              { review: "Best rental experience I’ve had in Phoenix. Transparent pricing and friendly service.", author: "— James R." },
              { review: "Loved the convenience and the clean car. Definitely booking here again!", author: "— Maria L." },
            ].map((r, idx) => (
              <div key={idx} className="p-4 bg-gray-50 dark:bg-slate-700 rounded-lg shadow-sm">
      ...
      <section className="section bg-gray-50 dark:bg-slate-800">
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {[
            { q: "Do I need a security deposit?", a: "No. We don’t require security deposits. Just bring your license and proof of full coverage insurance." },
            { q: "Can I rent if I only have liability insurance?", a: "Yes. You can upgrade to full coverage through us. If that doesn’t work, we can set you up securely through Turo." },
            { q: "What if I need to extend my rental?", a: "Extensions are simple. Just contact us, and we’ll adjust your booking with no hidden fees." },
            { q: "Do you allow out-of-state travel?", a: "Yes, but let us know in advance so we can confirm insurance coverage." },
            { q: "What’s your cancellation policy?", a: "Free cancellation, because we understand that your plans can change." },
          ].map((f) => (
            <div key={f.q}>
