...
export default function BookingBar() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      // Files
      const licenseFile = (formRef.current.querySelector(
        'input[name="license"]'
      ) as HTMLInputElement)?.files?.[0];
      const insuranceFile = (formRef.current.querySelector(
      ...
      if (insuranceFile)
        insurance_url = await uploadToBucket(insuranceFile, "insurance");
